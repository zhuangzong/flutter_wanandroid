...
class DragSortViewState extends State<DragSortView>
  ...
  void _init(BuildContext context, EdgeInsets padding, EdgeInsets margin) {
    ...
    for (int i = 0; i < _itemCount+1; i++) {
      ...
      double top = (space + _itemWidth*widget.aspectRatio) * (i ~/ 3);
      _positions.add(Rect.fromLTWH(left, top, _itemWidth, _itemWidth*widget.aspectRatio));
    }
  }
        ...
        return Positioned(
            ...
            child: Material(
              child: Container(
                ...
                height: _itemWidth*widget.aspectRatio + space * _zoomController.value * 2,
                child: overlay,
              ),
            ));
      });
    ...
    if (_cacheData.isEmpty) {
      ...
      for (int i = 0; i < _itemCount; i++) {
        ...
        children.add(Positioned(
          ...
          top: top,
          width: _itemWidth,
          height: _itemWidth*widget.aspectRatio,
          child: Offstage(
  ...
  Widget build(BuildContext context) {
    ...
    int row = ((_itemCount + 1) / 3).ceil();
    double realWidth =
        _itemWidth * column + widget.space * (column - 1) + padding.horizontal;
    double realHeight =
        _itemWidth*widget.aspectRatio * row + widget.space * (row - 1) + padding.vertical;
